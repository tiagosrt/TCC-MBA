...
predict(object = modelo4_Ca,dataset_pls_Cal) -> a
...
### Diagnóstico de heterocedasticidade via teste de Breusch-Pagan ###  AQUI!!
predict(object = modelo4_K,dataset_pls_Cal) -> a
dataset_pls_Cal$K.pls <- a[,,nc]
modelo7_K_lm <- lm(formula = K ~ K.pls , data = dataset_pls_Cal)

ols_test_breusch_pagan(modelo7_K_lm) # Esta funcao do pacote olsrr nao funciona para PLSR
...
### Grafico de pontos para Ca
 ...
 G1 <- Res_fit_PLS_Cal %>%
  ...
  geom_point(aes(x = Ca, y = Fit_M4_Ca, color = Field),
       ...
       labs(x = "Ca measured",
       y = "Ca predicted",
       title = "Calibration using PLS for Ca") +
  theme_bw() +
...
### Grafico de pontos para K
 ...
 G3 <- Res_fit_PLS_Cal %>%
  ...
  labs(x = "K measured",
       y = "K predicted",
       title = "Calibration using PLS for K") +
  theme_bw() +
